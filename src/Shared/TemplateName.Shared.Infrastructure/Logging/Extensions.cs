...
using TemplateName.Shared.Infrastructure.Logging.Decorators;
using TemplateName.Shared.Infrastructure.Logging.Options;
using Serilog;
using Serilog.Events;
using Serilog.Filters;

namespace TemplateName.Shared.Infrastructure.Logging;

public static class Extensions
{
    private const string ConsoleOutputTemplateName = "{Timestamp:HH:mm:ss} [{Level:u3}] {Message}{NewLine}{Exception}";
    private const string FileOutputTemplateName = "{Timestamp:HH:mm:ss} [{Level:u3}] ({SourceContext}.{Method}) {Message}{NewLine}{Exception}";
    ...
    private static void Configure(LoggerConfiguration loggerConfiguration, LoggerOptions options)
    {
        ...
        {
            loggerConfiguration.WriteTo.Console(outputTemplate: ConsoleOutputTemplateName);
        }
        ...
        if (fileOptions.Enabled)
        {
            ...
            loggerConfiguration.WriteTo.File(path, rollingInterval: interval, outputTemplate: FileOutputTemplateName);
        }
